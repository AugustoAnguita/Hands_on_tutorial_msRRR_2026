...
#    Before constructing these matrices, inspect extreme values and resolve
#    implausible values or influential outliers according to subject-matter
#    knowledge and a documented, analysis-specific rule. The pipeline does not
#    remove or winsorise outliers automatically because an appropriate decision
#    is case-specific.
#
...
#       automatically.
#    B) Lambda range: checks every candidate rank and issues a warning when its
#       CV-optimal lambda is an endpoint or its largest lambda still selects
#       exposures. The warning provides a tenfold-extended common lamseq, with
#       an integer length that preserves the log-scale resolution, for a manual
#       rerun of all ranks; it does not rerun the models automatically.
...
#
# 3) Uses independent rank/lambda initialisations (`warm = FALSE`) during model
#    selection. The whole-sample penalised refit is performed with
#    refit_msrrr(), reusing the exact selected initial values stored by
...
# msrrr_cv() learns X/Z and Gaussian-Y z-score standardisation using
...
# - With lamseq = NULL, the function first obtains a data-driven reference that
#   summarises the overall relationship between the exposures and outcomes.
#   It then creates nlam log-spaced values ranging from very strong to very weak
#   penalisation and uses exactly the same grid for all ranks and CV folds.
#   Technically, it sets
#   lambda_base = 1.1 * spectral_norm(crossprod(X, Y)) / n after preprocessing,
#   then tests a log-spaced grid from 10 * lambda_base down to
#   1e-4 * lambda_base. nlam is ignored when lamseq is supplied. Lambdas are
#   always ordered from largest to smallest before fitting.
# - After fitting, lambda coverage is checked separately for every rank. If a
#   rank-specific CV optimum is at an endpoint or the largest lambda still
#   selects exposures, the warning prints a proposed common lamseq for a manual
#   rerun of every rank. The function never expands and reruns it automatically.
#
# Warm-start guidance:
# - warm = FALSE is the workflow default. Every lambda starts independently
#   from the ridge/SVD initialisation, so results do not depend on the preceding
#   lambda solution or on the spacing of the path. This does not guarantee the
#   global optimum in the non-convex msRRR problem, but is easier to reproduce
#   and justify across analyses.
# - warm = TRUE reuses the preceding lambda solution within the same rank and
#   CV fold. It does not create data leakage and can be used when warm = FALSE
#   is computationally impractical, but results may depend more on the lambda
#   path. Report the setting used.
#
# CV and diagnostics:
...
fit_preg <- msrrr_cv(
  ...
  method = "CV", cv.criteria = cv.crit_preg, warm = FALSE,
...
fit_child <- msrrr_cv(
  ...
  nrankseq = rank_sequence, lamseq = NULL, nlam = n_lambda,
  foldid = foldid, nfold = num_folds,
  method = "CV", cv.criteria = cv.crit_child, warm = FALSE,
  diagnostics = TRUE, print_diagnostics_at_end = TRUE, progress = TRUE,
...
fit_comb <- msrrr_cv(
  ...
  nrankseq = rank_sequence, lamseq = NULL, nlam = n_lambda,
  foldid = foldid, nfold = num_folds,
  method = "CV", cv.criteria = cv.crit_comb, warm = FALSE,
  diagnostics = TRUE, print_diagnostics_at_end = TRUE, progress = TRUE,
...
#    Continuous outcomes and predictors are z-score standardised. Beta is the
#    change in outcome SDs associated with a 1-SD increase in a continuous
#    exposure, adjusted for covariates. Beta = 0.1 means an increase of 0.1
#    outcome SDs per 1-SD increase in the exposure.
#
...
#   2) recalculates X/Z and continuous-outcome z-score standardisation
...
if (run_penalized_loco) {
  ...
  for (cohort_index in seq_along(cohort_levels)) {
    ...
    Y_loco_raw <- Y[keep_subject, , drop = FALSE]
    Y_loco_raw[, "hs_bmi_c_cat"] <- as.numeric(
      Y_loco_raw[, "hs_bmi_c_cat"] >= 3
    )
    Y_loco <- scale_loco_outcomes(
      Y_loco_raw, outcome_families, family_mapping
    )
...
unpenalized_model_status <- do.call(rbind, list(
  unpenalized_refit_status(fit_preg_unpenalized, "Pregnancy"),
  unpenalized_refit_status(fit_child_unpenalized, "Childhood"),
  unpenalized_refit_status(fit_comb_unpenalized, "Combined")
))
write.csv2(
  unpenalized_model_status,
  file.path(results_dir, "UNPENALISED_MODEL_STATUS.csv"),
  row.names = FALSE
)
print(unpenalized_model_status)
...
unpenalized_loco_requested <- isTRUE(run_unpenalized_loco)
unpenalized_loco_bootstrap_requested <-
  isTRUE(run_unpenalized_loco_bootstrap)
...
unpenalized_loco_specifications <- list(
  Pregnancy = list(X_source = X_preg, whole_object = fit_preg_unpenalized),
  Childhood = list(X_source = X_child, whole_object = fit_child_unpenalized),
  Combined = list(
    X_source = cbind(X_preg, X_child), whole_object = fit_comb_unpenalized
  )
)
unpenalized_loco_available <- vapply(
  unpenalized_loco_specifications,
  function(x) is_unpenalized_refit_available(x$whole_object),
  logical(1)
)
unpenalized_loco_specifications <- unpenalized_loco_specifications[
  unpenalized_loco_available
]
run_unpenalized_loco <- unpenalized_loco_requested &&
  length(unpenalized_loco_specifications) > 0L
run_unpenalized_loco_bootstrap <- unpenalized_loco_bootstrap_requested &&
  run_unpenalized_loco
write.csv2(
  transform(
    unpenalized_model_status,
    included_in_unpenalized_loco = scenario %in%
      names(unpenalized_loco_specifications)
  ),
  file.path(unpenalized_loco_dir, "UNPENALISED_LOCO_MODEL_STATUS.csv"),
  row.names = FALSE
)

if (unpenalized_loco_requested && !run_unpenalized_loco) {
  message(
    ">>> [Optional LOCO] Unpenalised LOCO skipped: no whole-sample ",
    "penalised model selected any exposures."
  )
  unpenalized_loco_fits <- list()
  unpenalized_loco_results <- list()
  unpenalized_loco_summaries <- list()
  unpenalized_loco_pre_summaries <- list()
  unpenalized_loco_run_information <- data.frame()
  save(
    unpenalized_loco_fits, unpenalized_loco_results,
    unpenalized_loco_summaries, unpenalized_loco_pre_summaries,
    unpenalized_loco_run_information,
    file = file.path(unpenalized_loco_dir, "UNPENALISED_LOCO_RESULTS.RData")
  )
}
