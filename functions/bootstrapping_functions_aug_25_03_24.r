...
clusterEvalQ(cl, {
  ...
  source("./SCRIPTs/bootstrapping_functions_aug_25_03_24.r")  # Load custom functions from script
})
...
msrrr_bootstrap_k <- function(Y,X,Z, inds = NULL,family=NULL, familygroup=NULL,
      nrank=NULL, lambda=NULL) {

indsidx <- as.numeric(inds[[1]])

msrrr_res_boot <- msrrr.fit(Y = Y[indsidx,], X = X[indsidx,], Z = Z[indsidx,], family=family, familygroup=familygroup,nrank=nrank, lambda=lambda)
...
# old version
# msrrr_bootstrap_k <- function(blocks, inds = NULL,family=NULL, familygroup=NULL,
#                               nrank=NULL, lambda=NULL) {
#   
#   inds <- as.numeric(inds[[1]])
#   if (length(inds) > 0) {
...
#   }
#   
#   msrrr_res_boot <- msrrr.fit(Y = blocks[[3]], X = blocks[[1]], Z = blocks[[2]], family=family, familygroup=familygroup,nrank=nrank, lambda=lambda)
#   colnames(msrrr_res_boot$B) <- colnames(blocks[[3]])
#   rownames(msrrr_res_boot$B) <- colnames(blocks[[1]])
#   list_msrrr_res_boot <- lapply(names(as.data.frame(msrrr_res_boot$B)), function(col_name) {
#     matrix(as.data.frame(msrrr_res_boot$B)[,col_name], ncol = 1, dimnames =list(rownames(msrrr_res_boot$B), col_name))
#   })
#   
